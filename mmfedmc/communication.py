...
class Communication:
    def __init__(self, num_modalities, alpha_s=0.5, alpha_c=0.5):
        ...
        self.last_upload_rounds = torch.zeros(num_modalities, dtype=torch.int)  # 初始化每个模态的上次上传轮次

    def compute_shapley_values(self, predictions, true_labels):
        # 计算每个模态的Shapley值
        num_modalities = len(predictions)
        shapley_values = torch.zeros(num_modalities)
        ...
        for m in range(num_modalities):
            for subset in combinations(range(num_modalities), m):
                ...
                marginal_contribution = (prediction_with_m == true_labels).float().mean() - (prediction_without_m == true_labels).float().mean()
                shapley_values[m] += marginal_contribution / (torch.comb(torch.tensor(num_modalities - 1), torch.tensor(len(subset))) * num_modalities)
                
        return shapley_values
    
    def ensemble_predict(self, predictions, modalities):
        # 使用指定模态的预测进行集成预测
        combined_predictions = torch.mean(torch.stack([predictions[m] for m in modalities]), dim=0)
        return torch.round(combined_predictions)

    def select_modalities(self, shapley_values, model_size, recency, top_k):
        # 将Shapley值、模型大小和更新频率标准化
        normalized_shapley_values = (shapley_values - torch.min(shapley_values)) / (torch.max(shapley_values) - torch.min(shapley_values))
        normalized_model_size = (model_size - torch.min(model_size)) / (torch.max(model_size) - torch.min(model_size))
        normalized_recency = recency / torch.max(recency)
        ...
        selected_modalities = torch.argsort(priority)[-top_k:]
    ...
    def select_clients(self, local_losses, delta, K):
        ...
        top_losses_indices = torch.argsort(local_losses)[-threshold:]
        selected_clients = top_losses_indices[:K]
        return selected_clients

    def aggregate_predictions(self, predictions):
        # 聚合所有模态模型的预测结果
        mode = torch.mode(predictions, dim=0)[0]
        return mode
...
# model_size = torch.tensor([100, 200, 150])  # 示例模型大小
# recency = torch.tensor([communication.compute_recency(current_round=10, modality_index=i) for i in range(3)])
